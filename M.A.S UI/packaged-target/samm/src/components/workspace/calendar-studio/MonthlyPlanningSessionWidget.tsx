...
export function MonthlyPlanningSessionWidget({ data }: Props) {
  ...
  const handleAddEmphasis = () => {
    const nextValue = typeof window !== "undefined" ? window.prompt("Add a monthly emphasis") : null;
  ...
  const handleAddNote = () => {
    const nextValue = typeof window !== "undefined" ? window.prompt("Add a note or exception") : null;
    const trimmed = nextValue?.trim();
    if (!trimmed) return;
